...
var decoder_timing = [ [0.0, 0.02, 0.18, 0 ] , [0.38, 0.02, 0.08, 0] ];
var discard_timing = [ [0, 0.01, 0.01, 0 ] ];
var pc_read_timing = [ [0.00, 0.02, 0.08, 0 ] ];
var all_inject_timing = [ [0.01, 0.015, 0.0, 0.0 ] ];
var regen_timing = [ [0.0, 0.01, 0.04, 0, 0.5 ], [1.13, 0.01, 0.18, 0, 0.5], [1.3, 0.01, 0.02, 0, 1.5], [1.3, 0.01, 0.1, 0, 0.5], [1.5, 0.01, 0.1, 0, 0.5], [1.57, 0.01, 0.02, 0, 1.5] ];
...
var sub_holdoff_timing = [ [0.0, 0.01, 0.2, 0], [0.2, 0, 0.28, 0], [0.6, 0, 0.2, 0] ];
var sub2_holdoff_timing = sub_holdoff_timing;
var tst_holdoff_timing = [ [0.0, 0.01, 0.2, 0], [0.2, 0, 0.28, 0], [0.6, 0, 0.2, 0] ];
var hlt_holdoff_timing = [ [0.0, 0.01, 0.2, 0], [0.2, 0, 0.28, 0], [0.6, 0, 0.2, 0] ];
...
var address_send_release_timing = [ [0.41, 0.01, 0.02, 0 ] ];
var instruction_read_timing = [ [0.48, 0.01, 0.03, 0] ];
var instruction_reset_timing = [ [0.05, 0.03, 0, 0 ] ];
...
function create_memory(world, ground, xoffset, yoffset, part_index) {
	...
	compound_shape.m_vertices = union([line_shapes[7], blocker])[0];
...
function createWorld(world) {
    ...
    create_diagonal_channels(world, ground, 108, -190, -46.5+5, -28, channel_pitch, 0, 3, 4, 4.0);
    ...
    // Populate the memory
    for(var address=0;address < memory_data.length; address++) {
	for(var i=1;i<=8;i++) {
	    if (memory_data[address][i-1]==1) {
		create_fake_data(world, ground, channel_pitch*i+5.5, address*-4-1, 1);
	    }
	}
    }
}
